...
assemble_expr(Funs,Stack,_,{encode,TypeRep,A}) ->
    ...
    %%
    %% For boxed types the encoding is a heap fragment with relative
    %% addresses. For instance the tuple ("pink", (99, "apple")) would be encoded as
    %%    Offset   0   32   64     96    128    160   192     224
    %%    Word    64  128    4  "pink"    99    192     5  "apple"
    %%
    ...
    [ assemble_expr(Funs,Stack,nontail,A),         %% [ value ]
      %% The base pointer is the top of the heap
      aeb_opcodes:mnemonic(?MSIZE),                %% [ base value ]
      %% Call the encoder function. This returns 0 for boxed types, and `value`
      %% for unboxed types.
	...
	{funcall, {var_ref, encoder_name(TypeRep)},
      ...
      %% For boxed types we can simply discard the returned 0, but for unboxed
      %% types we need to actually write it to memory.
     case is_unboxed(TypeRep) of
        true  -> [ dup(2),                          %% [ base result base value ]
                   aeb_opcodes:mnemonic(?MSTORE) ]; %% [ base value ]   Mem[base] := value
        false -> aeb_opcodes:mnemonic(?POP)         %% [ base value ]
     end,
     pop_args(1)];                                  %% [ base ]
...
is_unboxed(word)       -> true;
is_unboxed({tuple, _}) -> false;
is_unboxed({list, _})  -> false;
is_unboxed(string)     -> false;
is_unboxed(function)   -> false.
